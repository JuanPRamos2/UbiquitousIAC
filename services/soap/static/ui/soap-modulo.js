(function (global) {
  ...
  function soapUrl() {
    ...
    if (el && el.getAttribute("data-soap-endpoint")) {
      return el.getAttribute("data-soap-endpoint");
    }
    return "/soap";
  }
  ...
  function invoke(xmlText) {
    try {
      ...
      xhr.open("POST", soapUrl(), false);
      xhr.setRequestHeader("Content-Type", "text/xml; charset=utf-8");
      xhr.send(xmlText);
      var ok = xhr.status >= 200 && xhr.status < 300;
      return { ok: ok, xml: xhr.responseText || "" };
    } catch (err) {
      try {
        return { ok: true, xml: handleRequest(xmlText) };
      } catch (inner) {
        if (inner && inner.fault) return { ok: false, xml: inner.xml };
        return {
          ok: false,
          xml: faultXml("Server", 500, "ServerFault", "El servicio no pudo completar la operación.")
        };
      }
    }
  }
  ...
  function reset() {
    try {
      var xhr = new XMLHttpRequest();
      xhr.open("POST", "/demo/reset", false);
      xhr.send();
    } catch (e) {}
